...
if ($_SESSION[SITE_CODE]['user']['permission'] < ALLOW_UPDATE) {
...
if (isset($_POST['btnSend'])) {
	...
	if (count($errors) == 0) {
		...
		db_update($db, 'news', $data, $_SESSION[SITE_CODE]['user']['acc']);
		redirect(SITE_URL.'Manage/News/');
	}
}
			...
			<div class='panel-body'>
				...
				<form class='form-horizontal' method='post'>
						...
						<div class='col-md-4'>
							<select class='form-control' name='is_enabled' reqired>
								<?php foreach ($enum['is_enabled'] as $key => $value): ?>
									<option value='<?=$key?>' <?php is_selected($key, $news['is_enabled']); ?>><?=$value?></option>
								<?php endforeach ?>
							</select>
						</div>
						...
						<div class='col-md-4'>
							<select class='form-control' name='priority' reqired>
								<?php foreach ($enum['priority'] as $key => $value): ?>
									<option value='<?=$key?>' <?php is_selected($key, $news['priority']); ?>><?=$value?></option>
								<?php endforeach ?>
							</select>
						</div>
					</div>
					<div class='form-group'>
						...
						<div class='col-md-10'>
							<input class='form-control' name='title' required placeholder='請輸入標題' value='<?=$news["title"]?>'>
						</div>
					</div>
					<div class='form-group'>
						<label class='col-md-2 control-label' for='content'>內文</label>
						<div class='col-md-10'>
							<textarea class='ckeditor' name='content'><?=str_replace('\\"', '', $news['content'])?></textarea>
						</div>
					</div>
					<div class='form-group'>
						<div class='col-md-offset-2 col-md-10'>
							<button class='btn btn-default' name='btnSend'>儲存修改</button>
						</div>
					</div>
				</form>
			</div>
